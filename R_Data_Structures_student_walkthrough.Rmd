...
```{r}
# to pull back specific things, R shows you in the list how they are indexed
...
student_name <- c("Mary", "Martha")
student_grade_level <- c("Freshman", "Senior")
student_class_grade <- c(88,91)
student_subject <- c("Art", "Choir")

df2 <- data.frame(student_name, 
                  student_grade_level, 
                  student_class_grade,
                  student_subject
                  )
...
```{r}
my_array <- array(
  c(124,216,166,254,123,923,12,14,2), 
  dim <- c(3,3,3)
)
my_array
```
...
my_array <- array(
  c(124,216,166,254,123,923,12,14,2), 
  dim <- c(3,3,3),
  dimnames <- list(c("Row 1", "Row 2", "Lunch"), c("Col 1", "Col 2", "Col 3"),
...
my_factor \<- factor(c("Blue", "Green", "Brown", "Hazel", "Gray", "Brown", "Blue"))
...
```{r}
my_factor <- factor(
  c("Blue", "Green", "Brown", "Hazel", "Gray", "Brown", "Blue"),
  levels <- c("Blue", "Gray", "Brown", "Hazel", "Green")
)
sort(my_factor)
...
4.  Print the second column of both matrices.
    ...
    print("In the above matrix that you created, print the value 5 by indexing in to the matrix.")
